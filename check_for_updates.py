...
def generate_changelog():
    matching_files = [
        ...
        if filename.startswith("hawker_data_") and filename.endswith(".json")
    ]
    ...
    if len(sorted_files) > 1:
        ...
        with open(f"./data/{sorted_files[1]}", "r") as file1, open(
            f"./data/{sorted_files[0]}", "r"
        ) as file2:
        ...
        for entry_id, prev_entry in enumerate(prev_data_records):
            new_entry = next(
                (
                    entry
                    for entry in new_data_records
                    if entry["serial_no"] == prev_entry["serial_no"]
                ),
                None,
            )
            ...
            if new_entry is None:
                ...
                changelog.append(
                    f"❌ **Removed**: {prev_entry['serial_no']} - {prev_entry['name']}"
                )
            else:
                # compare keys
                diff = []
                for key in prev_entry.keys():
                    if prev_entry[key] != new_entry[key]:
                        diff.append(
                            f"  - {key}: `{prev_entry[key]}` -> `{new_entry[key]}`"
                        )

                if diff:
                    changelog.append(
                        f"🔵 **Changed**: {entry_id + 1} {new_entry['name']}:\n"
                        + "\n".join(diff)
                    )
        ...
        for new_entry in new_data_records:
            prev_entry = next(
                (
                    entry
                    for entry in prev_data_records
                    if entry["serial_no"] == new_entry["serial_no"]
                ),
                None,
            )
            if prev_entry is None:
                changelog.append(
                    f"➕ **Added**: {new_entry['serial_no']} - {new_entry['name']}"
                )
